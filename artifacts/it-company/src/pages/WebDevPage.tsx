...
const INIT_PROBLEM = {
  ...
  cards: [
    { icon: XCircle,     title: "Template sites that look like everyone else",      desc: "Wix and Squarespace give you a cookie-cutter layout your competitors already have. Nothing about your brand, your product, or your customers." },
    { icon: AlertCircle, title: "Slow load times killing conversions",              desc: "A 1-second delay costs up to 7% of conversions. Bloated themes, unoptimised images, and shared hosting quietly bleed revenue every day." },
    { icon: XCircle,     title: "Zero visibility on Google",                        desc: "A beautiful site that nobody finds is a liability. Most agencies build first and think about SEO later — by which point the damage is already done." },
    { icon: AlertCircle, title: "No clear path from visitor to customer",           desc: "Without intentional UX, trust signals, and calls to action, visitors bounce. Your site should be your best salesperson — not a digital brochure." },
  ],
};
...
const INIT_SOLUTION = {
  ...
  cards: [
    { icon: Palette,   title: "100% Custom Design",        desc: "Every page is designed around your brand identity, audience, and goals. No templates, no recycled layouts — ever." },
    { icon: Search,    title: "SEO Built In From Day One",  desc: "Schema markup, Core Web Vitals, meta architecture, and sitemaps baked in before a single page goes live." },
    { icon: Smartphone,title: "Mobile-First, Always",       desc: "Designed and tested on real devices. Flawless across every screen — not just desktop-first with a responsive toggle." },
    { icon: Zap,       title: "Performance-Engineered",     desc: "Lazy loading, CDN delivery, image optimisation, and sub-second TTFB. We obsess over Lighthouse scores so you rank." },
    { icon: Shield,    title: "Secure & GDPR-Ready",        desc: "SSL, cookie consent, security headers, and privacy policy included. Your users' data is protected from launch." },
    { icon: RefreshCw, title: "30-Day Post-Launch Support",  desc: "We don't disappear after go-live. One month of free fixes, then flexible retainer packages to keep you growing." },
    { icon: BarChart3, title: "Analytics & Reporting",      desc: "GA4, Google Search Console, and monthly performance summaries so you always know what's working." },
    { icon: Bot,       title: "AI Features on Request",     desc: "Chat widgets, intelligent search, recommendation engines — layer AI on top when you're ready to scale." },
  ],
};
...
const INIT_TECH = {
  ...
  items: [
    { icon: Code2,        label: "React & Next.js",   desc: "Lightning-fast, SEO-optimised apps with server-side rendering and static generation." },
    { icon: Globe,        label: "WordPress",          desc: "Easy CMS with WooCommerce for blogs, catalogues, and content-heavy sites." },
    { icon: ShoppingCart, label: "Shopify",            desc: "Full-stack storefronts with payments, inventory, discount rules, and shipping built in." },
    { icon: Smartphone,   label: "Mobile-First PWA",   desc: "App-like experiences that install to home screens and work perfectly offline." },
    { icon: Bot,          label: "AI-Enhanced Sites",  desc: "Chatbots, recommendation layers, and intelligent search — we integrate what makes sense for your users." },
    { icon: Wrench,       label: "Any Stack You Need", desc: "Vue, Nuxt, Laravel, Django — we work in whatever fits your project and long-term team." },
  ],
};
...
const INIT_STATS = [
  { to: 320, suffix: "+", label: "Websites Delivered" },
  { to: 98,  suffix: "%", label: "On-Time Delivery" },
  { to: 9,   suffix: " days", label: "Avg. Time to Launch" },
...
const INIT_TESTIMONIALS = {
  ...
  items: [
    { quote: "OneSoft built our restaurant website in 8 days. Online reservations went from zero to 200+ bookings in the first month. Exceptional work.", name: "Khalid Mansour",  role: "Owner",    co: "Casa Bella Restaurant" },
    { quote: "We asked for a clinic website with an appointment system. They delivered something better than we imagined — on time and on budget.",        name: "Dr. Priya Sharma", role: "Director", co: "HealthFirst Clinic" },
    { quote: "The discovery call was 20 minutes. The site was live in 9 days. No agency has ever moved this fast without cutting corners.",               name: "James Okafor",  role: "Founder",  co: "Pixel & Oak Agency" },
  ],
};
...
function TechDrawer({ data, setData }: { data: typeof INIT_TECH; setData: (d: typeof INIT_TECH) => void }) {
  const [local, setLocal] = useState(data);
  const [saved, setSaved] = useState(false);
  const { closeSection } = useEditMode();
  function save() { setData(local); setSaved(true); setTimeout(() => setSaved(false), 2000); }
  return (
    <div className="space-y-4">
      ...
      <EField label="Subheading" value={local.subheading} onChange={v => setLocal(d => ({ ...d, subheading: v }))} multiline />
      <ESectionHeader>Tech Stack Items</ESectionHeader>
      {local.items.map((item, i) => (
        <div key={i} className="p-3 rounded-xl border border-gray-100 bg-gray-50 space-y-2">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Item {i + 1}</p>
          <EField label="Label" value={item.label} onChange={v => setLocal(d => ({ ...d, items: d.items.map((x, j) => j === i ? { ...x, label: v } : x) }))} />
          <EField label="Description" value={item.desc} onChange={v => setLocal(d => ({ ...d, items: d.items.map((x, j) => j === i ? { ...x, desc: v } : x) }))} multiline />
        </div>
      ))}
      <ESaveBar onSave={save} saved={saved} onClose={closeSection} />
    </div>
      ...
      {local.steps.map((s, i) => (
        <div key={i} className="p-3 rounded-xl border border-gray-100 bg-gray-50 space-y-2">
          <div className="flex items-center gap-2">
            <span className="text-xs font-black text-primary">Step {s.step}</span>
          </div>
      ...
      {local.map((s, i) => (
        <div key={i} className="p-3 rounded-xl border border-gray-100 bg-gray-50 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <EField label="Value (number)" value={String(s.to)} onChange={v => setLocal(d => d.map((x, j) => j === i ? { ...x, to: Number(v) || 0 } : x))} />
            <EField label="Suffix (e.g. +, %)" value={s.suffix} onChange={v => setLocal(d => d.map((x, j) => j === i ? { ...x, suffix: v } : x))} />
          </div>
  ...
  return (
    <div style={{ background: pageBg, minHeight: "100vh" }}>
      ...
      <EditableSection id="hero" label="Hero"
        drawer={<HeroDrawer data={hero} setData={setHero} />}>
            ...
            <div className="max-w-4xl mx-auto text-center">
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full mb-6"
                  ...
                  <MonitorSmartphone className="w-3 h-3" /> {hero.badge}
                </span>
              </motion.div>
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.08 }}
                ...
                {hero.headline.split("win customers").length > 1 ? (
                  <>{hero.headline.split("win customers")[0]}<span style={{ color: BLUE }}>win customers</span>{hero.headline.split("win customers")[1]}</>
                ) : hero.headline}
              </motion.h1>
      ...
      {/* PROBLEM */}
      <EditableSection id="problem" label="Problem Section"
        drawer={<ProblemDrawer data={problem} setData={setProblem} />}>
      ...
      {/* SOLUTION */}
      <EditableSection id="solution" label="Solution Section"
        drawer={<SolutionDrawer data={solution} setData={setSolution} />}>
      ...
      {/* TECH STACK */}
      <EditableSection id="tech" label="Tech Stack"
        drawer={<TechDrawer data={tech} setData={setTech} />}>
        <section className="pb-8 md:pb-10 lg:pb-[60px]" style={{ borderTop: `1px solid ${divider}` }}>
          <div className="container mx-auto px-4">
            <motion.div className="text-center mb-8"
              ...
              <h3 className="text-xl font-black mb-2" style={{ color: headingColor }}>{tech.heading}</h3>
              <p className="text-sm" style={{ color: t45 }}>{tech.subheading}</p>
            </motion.div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
              {tech.items.map((t, i) => (
                <motion.div key={t.label} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: i * 0.07 }}
                  className="flex gap-4 p-5 rounded-2xl" style={{ background: cardBg, border: `1px solid ${divider}` }}>
      ...
      {/* PROCESS */}
      <EditableSection id="process" label="Process / How It Works"
        drawer={<ProcessDrawer data={process} setData={setProcess} />}>
        <section className="py-8 md:py-10 lg:py-[60px]" style={{ background: sectionBg, borderTop: `1px solid ${divider}` }}>
          <div className="container mx-auto px-4">
      ...
      {/* STATS */}
      <EditableSection id="stats" label="Stats"
        drawer={<StatsDrawer data={stats} setData={setStats} />}>
      ...
      {/* INDUSTRIES */}
      <EditableSection id="industries" label="Industries"
        drawer={<IndustriesDrawer data={industries} setData={setIndustries} />}>
        <section className="py-8 md:py-10 lg:py-[60px]" style={{ background: sectionBg, borderTop: `1px solid ${divider}` }}>
          <div className="container mx-auto px-4">
      ...
      {/* TESTIMONIALS */}
      <EditableSection id="testimonials" label="Testimonials"
        drawer={<TestimonialsDrawer data={testimonials} setData={setTestimonials} />}>
        <section className="py-8 md:py-10 lg:py-[60px]" style={{ borderTop: `1px solid ${divider}` }}>
          <div className="container mx-auto px-4">
            <motion.div className="text-center max-w-xl mx-auto mb-12"
              initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
      ...
      {/* CTA */}
      <EditableSection id="cta" label="CTA Section"
        drawer={<CTADrawer data={cta} setData={setCta} />}>
